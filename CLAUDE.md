...
## Task: 상담사 라이브 응대 매뉴얼 AI (회사용) — 목 모드, 기존 채팅 재사용 (2026-07-17 완료)

목적: 상담사가 통화 중 즉시 참고할 수 있는 "고객 안내 요약"을 기존 소비자 조사 채팅 위에 얹는다. 새 조사 로직·새 파이프라인 없음 — role(consumer/agent)에 따라 답변 표시 레이어만 갈린다. scripts/의 (B) 파이프라인은 이번에도 불변. 로그인은 목(mock) 인증(값 검증 없음, 역할 전환 스위치일 뿐 보안 기능 아님).

**구현**
- `lib/role.ts`(신규): `mobiscope:role` localStorage 키 + `useSyncExternalStore` 기반 `useRole()` 훅. `loginAsAgent()`/`logout()`이 커스텀 이벤트를 dispatch해 같은 탭 안의 여러 컴포넌트(헤더 배지, 라우트 가드)가 즉시 반응하게 함.
- `pages/Login.tsx`(신규): "회사용 상담 지원 콘솔" 카피, 아이디·비밀번호 어떤 값이든 통과. `components/RequireAgent.tsx`(신규): role≠agent면 `/login`으로 리다이렉트.
- `App.tsx`: `/login`, `/agent-chat`(`<RequireAgent><Chat role="agent" /></RequireAgent>`) 라우트 추가 — `/chat`과 완전히 동일한 `Chat` 컴포넌트를 `role` prop만 다르게 재사용(새 페이지 로직 없음).
- `Layout.tsx`: role==='agent'일 때만 "상담사 모드" 배지 + 로그아웃 버튼.
- `lib/sse.ts`/`pages/Chat.tsx`: `streamChat()`이 `role`을 `POST /api/chat` 바디에 실어 보냄(기본값 consumer).
- `routers/chat.py`: `_campaign_parts()`가 이제 `remedy_type`도 함께 그룹핑(캠페인 내 항상 동일함을 DB로 확인 후 defect_cause와 동일한 방식으로 처리). `post_chat`이 body의 `role`(consumer/agent, 그 외 값은 안전하게 consumer로 취급)을 읽어 `_stream`→각 flow로 전달. ev6_cluster·ioniq5_charging 두 플로우 모두 이미 계산해 두었던 `parts`(캠페인별 그룹, 기존 "결함 부품 정보" 아코디언과 동일 데이터)를 role='agent'일 때만 `structured.agent_summary`로 한 번 더 실어 보냄 — 새 쿼리·새 데이터 없음, out_of_scope 플로우는 항상 `agent_summary: None`.
- `ChatAnswerCard.tsx`: `AgentSummaryBlock` 신규 — `agent_summary`가 있을 때만 답변 카드 맨 위(헤드라인보다 먼저)에 캠페인별 결함원인·시정방식(둘 다 원문 verbatim)·대상 부품(부품번호+공급사)을 나열, 맨 아래에 "고객에게 안내 시 참고용" 한 줄만 덧붙임(기존 하단 고지문은 그대로 유지, 새 사실 추가 없음).

**검증(요구된 5개 항목 전부 실측)**
1. role='consumer' EV6 시나리오: `agent_summary: null`, headline 등 나머지 구조는 role='agent' 응답과 완전히 동일(byte 비교 아님, 필드 비교로 확인).
2. role='agent' 동일 질문: `agent_summary` 2블록(24V200000·24V867000), 각 `defect_cause`/`remedy_type`/`part_number`(36400-1XFA0·36400-1XFA0QQK)/`supplier_canonical`(Hyundai Mobis)이 `parts` 테이블 직접 조회 결과와 정확히 일치. 브라우저 스크린샷으로 "고객 안내 요약 (상담사용)" 블록이 답변 최상단에 렌더링됨을 확인, 동일 질문을 consumer role로 재현 시 해당 블록이 아예 없음(count 0)을 확인.
3. 비로그인 상태로 `/agent-chat` 접근 → `/login` 리다이렉트 확인(Playwright, 단일 스크립트 내 결정적 재현).
4. 목 로그인 → 배지+로그아웃 버튼 노출 → 로그아웃 → role 초기화·배지 소멸·`/login`으로 이동까지 확인.
5. `npx tsc --noEmit` 무오류, `npm run build` 성공.

**버그 발견·수정 — 로그아웃 시 리다이렉트 경쟁 상태**: `Layout.tsx`의 `handleLogout`이 원래 `logout()` 호출 후 `navigate('/')`를 부르는 순서였는데, `logout()`이 dispatch하는 role 변경 이벤트를 `RequireAgent`(그 시점에 아직 `/agent-chat`에 마운트돼 있음)가 같은 틱에 감지해 자체적으로 `/login`으로도 리다이렉트를 시도하면서 두 네비게이션이 경쟁 — Playwright로 반복 테스트해 보니 약 50% 확률로 로그아웃 버튼 클릭이 아무 효과도 내지 못하는 것처럼 보이는 현상을 실측으로 재현. `navigate('/')`를 먼저 호출하고 `logout()`은 `setTimeout(…, 0)`으로 한 틱 늦춰 호출하도록 순서를 바꿔, 두 리다이렉트가 경쟁하더라도 항상 "role 없음 + 가드된 라우트 밖" 상태로 수렴하도록 수정. 수정 후 연속 15회 이상 재현 테스트로 안정성 확인.

**디버깅 중 얻은 교훈(기록해 둘 가치)**: 같은 세션에서 파일을 수정한 직후(수 초 이내) Playwright로 자동화 테스트를 돌리면 Vite HMR 리로드가 테스트 실행과 겹쳐 클릭이 씹히는 등 가짜 실패가 발생할 수 있음 — 실제로 위 로그아웃 버그를 처음 의심했을 때 이 HMR 타이밍 노이즈 때문에 몇 차례 잘못된 재현/미재현을 오갔다. 파일 수정 후에는 몇 초 대기하거나, 연속 여러 번 반복 실행해 일관성을 확인한 뒤에야 "진짜 버그"와 "테스트 환경 노이즈"를 구분할 수 있었다.

**한계(명시)**: "AI가 대화 로그를 학습해 예상 질문·정답지를 스스로 형성"하는 기능은 지시대로 구현하지 않음(로드맵 언급용). 실제 상담사 인증·통화 연동·CRM 연동 없음 — 전부 목 데모. `agent_summary`는 ev6_cluster·ioniq5_charging 두 시나리오에서만 채워짐(캠페인이 있는 경우), out_of_scope는 항상 null.
