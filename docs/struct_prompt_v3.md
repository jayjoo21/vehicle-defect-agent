...
> v2 대비 변경: 스키마에 `mentions_existing_recall` 필드 추가 (기존 리콜 언급 여부)
...
```json
{
  ...
  "mentions_existing_recall": "<true|false — 아래 [추가 규칙 ④] 참조>"
}
```
...
### [추가 규칙 ④] mentions_existing_recall (기존 리콜 언급 여부)

**규칙**: 서술문이 **이미 존재하는 리콜·시정조치·NHTSA 캠페인을 언급**하면 `true`, 아니면 `false`.

`true`로 판정하는 예:
- 특정 캠페인 번호 언급: "NHTSA Campaign Number: 24V107000", "recall 24V107000"
- 일반적 기존 리콜 언급: "this vehicle has several open recalls", "the dealer said this is under recall", "리콜 대상이라고 들었다"
- 기존 리콜의 부품 미수급·시정 지연에 대한 항의: "waiting for the recall part", "리콜 부품이 안 온다"

`false`로 판정하는 예:
- 리콜·캠페인·시정조치에 대한 언급이 전혀 없이 결함 증상만 서술한 경우

> **근거**: 이 프로젝트의 목표는 "리콜 이전"의 신규 결함 시그널 조기 탐지다. 신고 급증이 **기존 리콜에 대한 후속 항의**(부품 지연 등)일 수 있는데, 이를 신규 시그널로 오인하면 위양성(가짜 경보)이 된다. 이 필드는 급증 분석 시 "기존 리콜 여파" 성격의 신고를 분리하기 위한 것이다. severity·part_category 판정에는 영향을 주지 않는다(독립 필드).
