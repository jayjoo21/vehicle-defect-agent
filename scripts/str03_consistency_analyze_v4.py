#!/usr/bin/env python3
"""
str03_consistency_analyze_v4.py — STR-03 자기일관성(self-consistency) 분석 (v4판)

v3판(str03_consistency_analyze.py, ODINO 매칭·5개 정형필드)과의 차이:
- 매칭 키: odino → cmplid (v4는 CMPLID 단위 처리이므로)
- 비교 대상 정형필드 3개만 남음: severity, driving_context, insufficient_info
  (part_category는 v4에 없음 — compdesc1/compdesc2로 대체됐고, 이는 코드가 원본을
  그대로 파싱해 채우는 결정론적 필드라 "LLM이 흔들리는지" 측정 대상이 아니다.
  mentions_existing_recall도 v4 스키마 자체에 없음 — v3 전용이었다.)
- evidence_quote(위치 IoU)·symptoms(단어 자카드) 보조지표는 v3와 동일하게 유지.
- 표본 그룹(sample_group: A_multi_cmplid/B_unknown_with_sibling/C_unknown_no_sibling/
  RANDOM)별 불안정 비율도 추가로 집계 — v4 고유 메커니즘(다중 CMPLID 앵커링, 형제
  COMPDESC 조회)이 다른 케이스보다 더 흔들리는지 보기 위함.

방법론은 v3와 동일: 같은 100건을 동일 프롬프트(v4)로 temperature=0.4에서 3회 독립
실행한 결과를 비교 — 다른 LLM·사람 라벨과 비교하는 게 아니라 같은 모델이 스스로 얼마나
안정적으로 판단하는지 측정한다.
"""
import csv
import json
...
FIELD_LABEL = {
    "severity": "심각도", "driving_context": "주행 상황", "insufficient_info": "정보부족 여부",
}
...
def load_meta():
    """CMPLID → (정규화된 CDESCR, sample_group, compdesc1) 매핑."""
    with open(INPUT_CSV, encoding="utf-8-sig", newline="") as f:
        rows = list(csv.DictReader(f))
    cdescr_map, group_map, compdesc1_map = {}, {}, {}
    for r in rows:
        cmplid = str(r["CMPLID"])
        cdescr_map[cmplid] = normalize_text(fix_mojibake(r["CDESCR"]))
        group_map[cmplid] = r.get("sample_group", "")
        compdesc1_map[cmplid] = r["COMPDESC"].split(":")[0].strip()
    return cdescr_map, group_map, compdesc1_map
...
def main():
    runs = load_runs()
    cdescr_map, group_map, compdesc1_map = load_meta()
    common_ids = sorted(set(runs[0]) & set(runs[1]) & set(runs[2]), key=int)
    missing = (set(runs[0]) | set(runs[1]) | set(runs[2])) - set(common_ids)
    if missing:
        print(f"경고: 3회 전부에 존재하지 않는 CMPLID {len(missing)}건 제외: {sorted(missing)}")

    per_field_hist = {f: Counter() for f in DISCRETE_FIELDS}
    details = []

    for cmplid in common_ids:
        recs = [runs[k][cmplid] for k in range(3)]
        field_unique, field_values = {}, {}
        ...
        cdescr_norm = cdescr_map.get(cmplid, "")
        ...
        details.append({
            "cmplid": cmplid,
            "sample_group": group_map.get(cmplid, ""),
            "compdesc1": compdesc1_map.get(cmplid, ""),
    ...
    group_tier = Counter((d["sample_group"], d["tier"]) for d in details)
    group_totals = Counter(d["sample_group"] for d in details)
    ...
    with open(OUT_CSV, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["cmplid", "sample_group", "compdesc1", "instability_score", "tier"]
                            + DISCRETE_FIELDS + ["evidence_overlap_iou", "symptom_overlap_jaccard"])
        w.writeheader()
    ...
    lines = []
    lines.append("# STR-03 v4 자기일관성(Self-Consistency) 분석 리포트\n")
    lines.append("## 개요")
    lines.append(f"- 대상: {n}건 (동일 100건을 v4 프롬프트, temperature=0.4로 3회 독립 실행, CMPLID 매칭)")
    lines.append("- 비교 대상 정형필드 3개: severity·driving_context·insufficient_info "
                 "(v3의 part_category는 v4엔 없음=compdesc1/compdesc2로 대체, 코드가 채우는 "
                 "결정론적 필드라 비교 불필요. mentions_existing_recall도 v4 스키마엔 없음)")
    ...
    lines.append("## 필드별 3회 일치율 (정형값 3개)")
    ...
    lines.append("## 레코드별 불안정 점수 분포 (3개 필드 중 일치하지 않는 필드 수)")
    ...
    lines.append("## 표본 그룹별 불안정 비율 (v4 고유 메커니즘 검증)")
    lines.append("| 그룹 | 안정 | 부분 흔들림 | 불안정 | 총 | 불안정+부분흔들림 비율 |")
    lines.append("|---|---|---|---|---|---|")
    for g in ["A_multi_cmplid", "B_unknown_with_sibling", "C_unknown_no_sibling", "RANDOM"]:
        tot = group_totals.get(g, 0)
        if tot == 0:
            continue
        st = group_tier.get((g, "안정"), 0)
        pt = group_tier.get((g, "부분 흔들림"), 0)
        ut = group_tier.get((g, "불안정"), 0)
        shaky_ratio = (pt + ut) / tot if tot else 0
        lines.append(f"| {g} | {st} | {pt} | {ut} | {tot} | {shaky_ratio:.1%} |")
    lines.append("")
    lines.append("> A_multi_cmplid(다중 CMPLID 앵커링)·B_unknown_with_sibling(형제 COMPDESC 조회) "
                 "그룹의 불안정 비율이 RANDOM보다 뚜렷이 높다면, 이 두 메커니즘 자체가 판단 흔들림을 "
                 "더 유발한다는 뜻으로 해석 가능 — 다만 각 그룹 표본이 13~27건으로 작아 확정적 결론은 "
                 "아니다.\n")
    ...
    lines.append(f"- 안정 레코드(3필드 전부 일치, n={len(stable)}) 평균 evidence 겹침(IoU): "
    ...
    lines.append("이 실험에서 관측된 흔들림은 **전부 temperature=0.4로 일부러 올린 결과다.** 이건 "
                 "숨은 결함이 아니라 self-consistency 진단 방법 자체의 설계 원리 — \"살짝 흔들어서 "
                 "어느 판단이 약한 다리인지 본다\"는 것이라, 흔들림이 온도 때문에 발생하는 것 자체는 "
                 "의도된 동작이다. 다만 이로 인해 이 데이터로 정당하게 주장할 수 있는 것과 없는 것이 "
                 "갈린다.\n")
    lines.append("- **정당한 주장 — 상대적 순위**: T=0.4에서도 3필드 전부 그대로인 레코드는 모델이 "
                 "강하게 확신하는 케이스다. 반대로 흔들린 레코드는 온도를 얼마로 잡든 상대적으로 더 "
                 "흔들릴 가능성이 높은 후보군 — 이 순위를 \"사람이 먼저 봐야 할 우선순위\"로 쓰는 것은 "
                 "타당하다.")
    lines.append("- **부당한 주장 — 절대 수치**: 위 필드별 일치율을 \"Gemini 운영(temperature=0) "
                 "판정이 이 정도로 불안정하다\"고 읽으면 안 된다. T를 더 올렸으면 이 숫자는 더 "
                 "나빠졌을 것이고, 이는 모델의 실제 신뢰도가 아니라 이번에 고른 T=0.4라는 진단 강도를 "
                 "반영하는 수치다. 이 실험은 T=0(운영값) 3회 반복이라는 대조군 없이 진행됐다.")
    lines.append("- **표본 그룹별 비교도 참고 가설**: 위 '표본 그룹별 불안정 비율'은 각 그룹 13~27건 "
                 "표본에서 나온 관찰이라, 확정된 결론이 아니라 추가 검증이 필요한 가설로 취급한다.")
    lines.append("- **결론**: 이번 실험의 확실한 산출물은 \"이 100건 중 어디를 먼저 사람이 봐야 "
                 "하는가\"라는 우선순위 리스트(상세 CSV)이고, 절대 수치나 그룹 간 비교 해석은 참고 "
                 "가설로만 취급한다.\n")
    ...
    lines.append("| CMPLID | 그룹 | compdesc1 | 점수 | 등급 | severity(3회) | driving_context(3회) | "
                 "insufficient_info(3회) | evidence IoU | symptom 자카드 |")
    lines.append("|---|---|---|---|---|---|---|---|---|---|")
    for d in top:
        lines.append(f"| {d['cmplid']} | {d['sample_group']} | {d['compdesc1']} | {d['instability_score']} | "
                     f"{d['tier']} | {d['severity']} | {d['driving_context']} | {d['insufficient_info']} | "
                     f"{d['evidence_overlap_iou']} | {d['symptom_overlap_jaccard']} |")
    lines.append("")
    ...
    lines.append("- symptom 자카드는 공백 기준 단어 집합 비교라 같은 뜻을 다른 단어로 쓴 "
                 "경우는 실제보다 낮게 나올 수 있음 — 정답 판정이 아니라 참고 지표로만 사용.")
    lines.append("- evidence_quote 위치를 못 찾은 건(원문 부분매칭 실패, 예: insufficient_info=true "
                 "+ 빈 인용)은 겹침 계산에서 제외됨.")
